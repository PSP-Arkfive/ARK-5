...
void patch_update_plugin_module(SceModule* mod)
{
    ...
    version = sctrlSEGetVersion() << 24 | sctrlHENGetVersion() << 16 | sctrlHENGetMinorVersion() << 8;	// ARK's full version number
...
void patch_SceUpdateDL_Library(SceModule* mod)
{
    ...
    for (u32 addr = text_addr; addr<top_addr; addr++){
        if (strcmp((char*)addr, "http://") == 0){
        	memset((char*)addr, 0, 84);
        	sprintf((char*)addr, "%s/psp-updatelist.txt?", server);
        	break;
        }
    }
}
